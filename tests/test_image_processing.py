...
from arknights_pixel.image_processing import matrix_to_image, resize_to_24
from arknights_pixel.palette import (
    N,
    OKLAB_PALETTE,
    PALETTE,
    color_distance,
...
class ImageProcessingTests(unittest.TestCase):
    def test_resample_strength_endpoints(self):
        ...
        nearest = resize_to_24(
            source,
            "crop",
            "Nearest（像素画）",
            resample_strength=1.0,
        )
        zero_strength = resize_to_24(
            source,
            "crop",
            "Lanczos（原版）",
            resample_strength=0.0,
        )
        full_lanczos = resize_to_24(
            source,
            "crop",
            "Lanczos（原版）",
            resample_strength=1.0,
    ...
    def test_default_match_strength_preserves_selected_algorithm(self):
        ...
        for color in samples:
            ...
            value = rgb_to_oklab(color)
            lab_expected = min(
                range(len(PALETTE)),
                key=lambda index: sum(
                    (value[channel] - OKLAB_PALETTE[index][channel]) ** 2
                    for channel in range(3)
                ),
            )
            self.assertEqual(
                rgb_expected,
                nearest_palette_index(color, perceptual=False, match_strength=1.0),
            )
            self.assertEqual(
                lab_expected,
                nearest_palette_index(color, perceptual=True, match_strength=1.0),
            )
    ...
    def test_official_share_grid_recovers_cells_under_uid_overlay(self):
        ...
        for row in range(N):
            matrix_row = []
            for col in range(N):
                index = (row // 4 * 5 + col // 5) % len(PALETTE)
        ...
        recovered, bounds, confidence = detect_official_share_grid(image)
    ...
    def test_matrix_to_image_is_exact_24_square(self):
        matrix = [[(x + y) % len(PALETTE) for x in range(N)] for y in range(N)]
        image = matrix_to_image(matrix)
        self.assertEqual((24, 24), image.size)
        self.assertEqual(PALETTE[matrix[7][9]], image.getpixel((9, 7)))
