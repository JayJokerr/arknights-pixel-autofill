...
| Windows 显示缩放 | **100%** |
| 游戏 UI 缩放 | **100%** |
| 游戏客户区分辨率 | **1280×720（推荐）** |
| 窗口模式 | 窗口化或无边框窗口，不要跨两块屏幕 |
| 界面状态 | 24×24编辑画布和右侧调色板完整可见 |

工具内置了网格自动校准和多显示器坐标处理，但由于游戏UI、Windows DPI和显卡缩放方式可能同时影响实际坐标，`100% + 1280×720` 仍是推荐组合。
...
3. 找到 **缩放** 或 **缩放与布局**。
4. 将运行游戏的显示器设为 **100%**。
5. 如果使用双屏，建议两块屏幕都设为100%，然后重新启动游戏和本工具。
